...
export default function ProductsPage() {
  ...
  const simulate = async () => {
    ...
    const body: any = { input: simInput };
    if (policy?.id) body.policyId = policy.id;
    const res = await fetch(`${GATEWAY}/policies/simulate`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) });
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          <label className="block md:col-span-2">
            <div className="text-xs text-gray-600 mb-1">Policy</div>
            <select className="border rounded-xl px-3 py-2 w-full" value={policyId} onChange={e => setPolicyId(e.target.value)}>
              {policies.map(p => <option key={p.id} value={p.id}>{p.name} ({p.version})</option>)}
            </select>
            {currentPolicy && (
              <div className="text-[10px] text-gray-500 mt-1 break-all">FP: {currentPolicy.fingerprint}</div>
            )}
          </label>
